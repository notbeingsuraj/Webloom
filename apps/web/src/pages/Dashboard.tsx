...
const surface = 'rounded-2xl border border-border bg-card p-6 shadow-sm';
    ...
    <div className="space-y-8">
      <header className={`${surface} md:p-8`}>
        ...
        <div className="mt-8 border-t border-border pt-6">
          <p className="text-sm text-muted-foreground">Your pipeline</p>
            ...
            {metrics.map(({ name, value, detail, icon: Icon }) => (
              <div key={name} className={card}>
                ...
                <div className="mt-5 flex items-end justify-between gap-3">
                  <span className="text-3xl font-semibold tracking-[-0.05em] text-foreground">{value}</span>
                  <span className="text-[11px] uppercase tracking-[0.12em] text-muted-foreground">Live</span>
                </div>
      ...
      <section className={`${surface} md:p-7`}>
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold tracking-[-0.05em] text-foreground">Priority Leads</h2>
          </div>
          <Link to="/leads/new" onClick={() => analytics.primaryCta('pipeline_review')} className="text-sm font-medium text-primary hover:text-primary/80">
            Review pipeline
          </Link>
        </div>

        <div className="overflow-x-auto rounded-2xl border border-border">
          <div className="hidden min-w-[720px] grid-cols-[1.5fr_1fr_1fr_0.8fr_0.8fr_0.9fr] gap-3 bg-secondary px-4 py-3 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground md:grid">
            <span>Business</span>
            <span>Category</span>
            <span>Location</span>
            <span>Score</span>
            ...
            {leadsData?.length ? (
              leadsData.map((lead: any) => (
                <div key={lead._id} className="grid min-w-[720px] gap-3 px-4 py-4 md:grid-cols-[1.5fr_1fr_1fr_0.8fr_0.8fr_0.9fr] md:items-center">
                  <div>
                    <p className="text-sm font-medium text-foreground">{lead.businessName || 'Local business'}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{lead.location?.city || 'Local business'}</p>
                  </div>
                  <div className="text-sm text-muted-foreground">{lead.businessCategory || 'N/A'}</div>
                  <div className="text-sm text-muted-foreground">{lead.location?.city || 'N/A'}</div>
                  <div className="text-sm font-medium text-foreground">{lead.opportunityScore?.total ?? '—'}</div>
                  <div><StatusBadge status={lead.status || 'new'} /></div>
                  <div className="md:text-right">
                    <Link
                      to={`/leads/${lead._id}`}
                      className="inline-flex items-center justify-center rounded-full bg-primary px-3 py-2 text-xs font-medium text-primary-foreground transition hover:bg-primary/90"
                    >
                      View
                    </Link>
                  </div>
